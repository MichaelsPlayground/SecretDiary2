...
public class App_v1 extends Application implements Application.ActivityLifecycleCallbacks {
    private static App_v1 instance;
    ...
    void onAppForeground(Activity activity) {
        if (hasBiometricProtection() && activity instanceof FragmentActivity) {
            Log.d("SecretDiary", "before show biometric prompt");
    ...
    public static void showBiometricPrompt(FragmentActivity activity) {
        ...
        BiometricPrompt prompt = new BiometricPrompt(activity, new BiometricPrompt.AuthenticationCallback() {
            ...
            public void onAuthenticationSucceeded(@NonNull BiometricPrompt.AuthenticationResult result) {
                App_v1.instance.isLocked = false;
                App_v1.instance.unlock(activity);
            }
        ...
        Log.d("SecretDiary", "before promptInfo");
        ...
        Log.d("SecretDiary", "before authenticate (promptInfo)");
    ...
    public void onActivityResumed(@NonNull Activity activity) {
        // ergänzt gem Mail vom Autor, Andreas Linke
        if (countStarted == 1) {
            // App geht in den Vordergrund
            Log.d(getClass().getSimpleName(), "App comes into foreground");
            onAppForeground(activity);
